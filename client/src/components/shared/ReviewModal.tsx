...
export default function ReviewModal({ letterId, open, onOpenChange }: ReviewModalProps) {
  ...
  const claimMutation = trpc.review.claim.useMutation({
    onSuccess: () => { toast.success("Letter assigned to you", { description: "You can now review and edit the draft." }); invalidate(); },
    onError: (e: any) => toast.error("Could not claim letter", { description: e.message }),
  });
  const saveMutation = trpc.review.saveEdit.useMutation({
    onSuccess: () => {
      toast.success("Draft saved", { description: "Your edits have been preserved." });
  ...
  const approveMutation = trpc.review.approve.useMutation({
    onSuccess: () => {
      toast.success("Letter approved", { description: "The subscriber has been notified and can now download the final PDF." });
  ...
  const rejectMutation = trpc.review.reject.useMutation({
    onSuccess: () => {
      toast.success("Letter rejected", { description: "The subscriber has been notified of the decision." });
      setActiveAction(null);
      invalidate();
    },
  ...
  const changesMutation = trpc.review.requestChanges.useMutation({
    onSuccess: () => {
      toast.success("Revision requested", { description: "The subscriber has been asked to provide additional information." });
      setActiveAction(null);
      invalidate();
    },
  ...
  const handleSave = () => {
    ...
    if (plainText.length < 50) {
      toast.error("Draft too short", { description: "The letter must contain at least 50 characters before saving." });
      return;
    }
  ...
  const handleApprove = () => {
    ...
    if (plainText.length < 50) {
      toast.error("Cannot approve", { description: "The letter must contain at least 50 characters before approval." });
      return;
    }
